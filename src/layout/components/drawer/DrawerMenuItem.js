...
export default function SideberMenuItem({ typeMenu, isOpen, to, title, icon, dataMenu, index, toggleDrawer }) {
    ...
    if (typeMenu == "main") {
        return (_jsxs("div", { onClick: (e) => {
                handleClick(to);
                toggleDrawer(e);
            }, className: `flex items-center text-sm gap-3.5 font-medium p-2 my-2 hover:bg-blue-100 hover:scale-110 rounded-md ${isActive && `bg-blue-200`}`, children: [_jsx("div", { children: _jsx("i", { className: `${icon} fs-2` }) }), _jsx("label", { style: { transitionDelay: `${index + 1}00ms` }, className: `whitespace-pre duration-500 text-lg ${!isOpen && "opacity-0 translate-x-28 overflow-hidden"}`, children: title })] }));
    }
    if (typeMenu == "sub") {
        return (_jsxs("div", { onClick: (e) => {
                handleClick(to);
                toggleDrawer(e);
            }, className: `flex items-center text-sm gap-3.5 font-medium p-2 my-2 hover:bg-blue-100 rounded-md ${isActive && `bg-blue-200`}`, children: [_jsx("div", { children: _jsx("i", { className: `${icon} fs-2` }) }), _jsx("label", { className: "text-lg", children: title })] }));
    }
}
